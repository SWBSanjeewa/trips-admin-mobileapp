...
import BusPhotosAddCard from "../../service-bus/bus-photos-add/extra/busphotos-add-card.component";
...
const BusAdd = ({ navigation }): React.ReactElement => {
	...
	const onNavigateToJourney = (): void => {
		...
		navigation.navigate("TourBusStoppingsScreen")
	};
		...
		<ScrollView>
			...
			<View>
				...
				onPress={(data, details = null) => {
					...
					if(selectedStopping != ""){
						var index = appStore.bus.getIndex(selectedStopping);
						appStore.bus.addStoppingAtIndex(address,index+1);
						setSelectedStopping("");
					}else{
						appStore.bus.addStopping(address);
					}
