...
const BusStoppingEdit = ({ navigation }): React.ReactElement => {
	...
	useEffect(() => {
		...
		if(initialized==false){	
			if(route.params.id == "routebus-stopping-edit-edit"){	
	...
	const onSetStartLocationPress = (): void => {
		...
		navigation && navigation.navigate("LocationAdd", {id: "routebus-stopping-edit-edit","journeyType": route.params.journeyType ,latitude:latitude , longitude:longitude, placeName:place,  oldLatitude: oldLatitude, oldLongitude: oldLongitude, time: time, returnroute: "RouteBusStoppingEditEditScreen", parentReturnRoute: route.params.returnRoute, duration: duration, index: route.params.index});
	};

	const onUpdatePress = async(): void => {
		...
		if(isValidValues()){
			...
			if(route.params.journeyType == "RouteBusJourney"){
				console.log("### "+`/routebuses/`+appStore.routeBus.objectId+`/journey/stoppings/`+index+`/update`);
				try {
					const response: AxiosResponse = await client.put(`/routebuses/`+appStore.routeBus.objectId+`/journey/stoppings/`+route.params.index+`/update`, data, config);
					if(response.status == 200){
						appStore.routeBus.journey.updateStopping(oldLatitude, oldLongitude, place,latitude,longitude,duration);
						navigation.navigate('RouteBusJourneyStoppingsListEdit',{"journeyType": route.params.journeyType});
					}	
				} catch(err) {
					console.log(err);
				}
				...
				try {
					const response: AxiosResponse = await client.put(`/routebuses/`+appStore.routeBus.objectId+`/returnJourney/stoppings/`+route.params.index+`/update`, data, config);
					if(response.status == 200){
	...
	const onStoppingDeletePress = async(): void => {
		const config: AxiosRequestConfig = {
			headers: {
				'Accept': 'application/json',
				'token': appStore.user.accessToken
		...
		if(route.params.journeyType == "RouteBusJourney"){
			try {
				const response: AxiosResponse = await client.put(`/routebuses/`+appStore.routeBus.objectId+`/journey/stoppings/`+index+`/delete`, config);
				if(response.status == 200){
			...
			try {
				console.log(`/routebuses/`+appStore.routeBus.objectId+`/returnJourney/stoppings/`+index+`/delete`);
				const response: AxiosResponse = await client.put(`/routebuses/`+appStore.routeBus.objectId+`/returnJourney/stoppings/`+index+`/delete`, config);
				if(response.status == 200){
	...
	const onAddStoppingPress = (): void => {
		...
		navigation && navigation.navigate("RouteBusStoppingAddEditScreen",{ "index": route.params.index+1, "journeyType": route.params.journeyType});
	}
	...
	return (
		...
		<ScrollView>
			{route.params?.journeyType == "RouteBusReturnJourney" && (
				<TopNavigation accessoryLeft={renderBackAction} title="Edit - Edit Return Stopping"/>
			)}
			{route.params?.journeyType == "RouteBusJourney" && (
				<TopNavigation accessoryLeft={renderBackAction} title="Edit - Edit Stopping"/>
			)}
