...
const BusStoppingAdd = ({ navigation }): React.ReactElement => {
	...
	useEffect(() => {
		if(initialized == false){
			if(route.params){
				if(route.params.id == "routebus-stopping-add-edit"){
	...
	const onSetStartLocationPress = (): void => {
		...
		navigation && navigation.navigate("LocationAdd", {id: "routebus-stopping-add-edit", "journeyType": route.params.journeyType,latitude:latitude , longitude:longitude, placeName:place, returnroute: "RouteBusStoppingAddEditScreen" ,index: index , duration: duration});
	};
	...
	const onAddPress = async(): void => {
		if(isValidValues()){
			...
			if(place !=null && journeyType == "RouteBusJourney"){
				if(index==0){
					try {
						const response: AxiosResponse = await client.put(`/routebuses/`+appStore.routeBus.objectId+`/journey/stoppings/add`, data, config);
						if(response.status == 200){
							appStore.routeBus.journey.addStopping(place,latitude,longitude,duration);
						}	
					} catch(err) {
						console.log(err);
					}	
					...
					try {
						const response: AxiosResponse = await client.put(`/routebuses/`+appStore.routeBus.objectId+`/journey/stoppings/add/`+index, data, config);
						if(response.status == 200){
							appStore.routeBus.journey.addStoppingAtIndex(place,latitude,longitude,index,duration);
						}	
					} catch(err) {
						console.log(err);
					}	
				...
				if(index==0){
					try {
						const response: AxiosResponse = await client.put(`/routebuses/`+appStore.routeBus.objectId+`/returnJourney/stoppings/add`, data, config);
						if(response.status == 200){
							appStore.routeBus.returnJourney.addStopping(place,latitude,longitude,duration);
						}	
					} catch(err) {
						console.log(err);
					}
					...
					try {
						const response: AxiosResponse = await client.put(`/routebuses/`+appStore.routeBus.objectId+`/returnJourney/stoppings/add/`+index, data, config);
						if(response.status == 200){
							appStore.routeBus.returnJourney.addStoppingAtIndex(place,latitude,longitude,index,duration);
						}	
					} catch(err) {
						console.log(err);
					}
	...
	return (
		...
		<ScrollView>
			...
			<View style={{flex: 1,flexDirection: "row", justifyContent: "space-between"}}>
				<Button style={styles.actionButton} onPress={()=>onAddPress()}>
				Add
				</Button>
			</View>
