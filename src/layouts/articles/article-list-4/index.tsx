...
export default ({ navigation }): React.ReactElement => {
	...
	const renderHeadingItem = (): React.ReactElement => (
		<ImageOverlay style={styles.headingArticleContainer} source={headingArticle.image}>
			<Text style={styles.headingArticleTitle} status="control" category="h3">
				{headingArticle.title}
			</Text>
			<Text style={styles.headingArticleDescription} category="h6" status="control">
				{headingArticle.description}
			</Text>
			<Button style={styles.readButton} status="control" onPress={() => onItemPress(0)}>
				READ
			</Button>
		</ImageOverlay>
	);
