...
const TransportserviceDriversList = ({ navigation }): React.ReactElement => {
	...
	function getPhoneNumberInternal() {
		...
		return selectContactPhone()
			.then(selection => {
				...
				appStore.transportService.addDriver(contact.name,phNo);
	...
	const onAddContactListShowPress = () => {
		...
		//navigation.navigate("ContactList", { "action": "add-driver"});
		getPhoneNumber();
	};
	...
	*/
	...
	const onAddButtonPress = (): void => {
		if(isValidValues()){
			appStore.transportService.addDriver(name, mobileNumber);
			resetValues();
	...
	const onDeletePress = (): void => {
		appStore.transportService.deleteDriver(mobileNumber);
		resetValues();
	...
	const onEditPress = (): void => {
		if(isValidValues()){
			appStore.transportService.updateDriver(name, mobileNumber);
			resetValues();
			refRBSheetEdit.current.close();
		}
	};
	...
	return (
		...
		<SafeAreaLayout style={styles.container} insets="top">
			<TopNavigation title={props => (
				...
				{appStore.transportService.drivers.length == 0 && 
					<Text style={{paddingLeft: 18}}>No drivers created yet</Text>
				}
				...
				<RBSheet draggable dragOnContent key="busDriverAdd" ref={refRBSheetAdd} height={300}>
					<View style={{ paddingHorizontal: 10}}>
						<View style={{ flexDirection: "row",  justifyContent: 'center' , padding: 5, margin: 5}}>
							<RNText style={{ fontWeight: "500", fontSize: 18}}>Add Driver</RNText>
						</View>
...
export default observer(TransportserviceDriversList);
