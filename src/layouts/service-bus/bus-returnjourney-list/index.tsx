...
const BusReturnJourneyList = ({ navigation }): React.ReactElement => {
	...
	const [copyFromDisabled, setCopyFromDisabled] = React.useState(appStore.bus.returnJourney.stoppings.length >0 );
	

	const onStoppingEditPress = (stopping,index): void => {
		navigation && navigation.navigate("BusStoppingEditScreen", {id: "stopping-edit", "journeyType": "ReturnJourney","returnRoute": "ReturnJourney", oldLatitude: stopping.latitude, oldLongitude: stopping.longitude, place: stopping.place, latitude: stopping.latitude, longitude: stopping.longitude, time: stopping.time, index: index});
	};


	const onStoppingAddPress = (): void => {
		appStore.bus.setJourneyRunningDays(runningDays.toString());
		navigation && navigation.navigate("BusStoppingAddScreen",{ "journeyType": "ReturnJourney"});
	};
	...
	const onCopyFromChecked = (): void => {
		...
		var tempStoppings = Array.from(appStore.bus.stoppings);;
		tempStoppings.reverse().forEach(stopping => {
			console.log(">>"+stopping);
            appStore.bus.addReturnJourneyStopping(stopping.place,stopping.latitude,stopping.longitude,"00.00 AM");
        });
	...
	useEffect(() => {
			if(initialized == false){
			  var returnJourneyWorkdaysNumbers = appStore.bus.returnJourney.runningDays.split(',').map(function(item) {
				return parseInt(item, 10);
			  });
			  setRunningDays(returnJourneyWorkdaysNumbers);
			  setInitialized(true);
			}
